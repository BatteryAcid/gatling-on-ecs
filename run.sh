...
do  
   aws s3 cp ${_dir}simulation.log s3://${REPORT_BUCKET}/$HOSTNAME-simulation.log
done

#Just a hack to make container wait and 
# not terminate itself causing services to start new tasks from ASG
echo "Wait until terminate?"
